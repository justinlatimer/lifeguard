...
mod sync_tests {
  ...
  fn test_deref_arc() {
      let str_pool : Pool<String> = Pool::with_size(1);
      let rstring = str_pool.new_arc_from("cat");
      assert_eq!("cat", *rstring);
  }
  ...
  #[test]
  fn test_deref_mut_arc() {
      let str_pool : Pool<String> = Pool::with_size(1);
      let mut rstring = str_pool.new_arc_from("cat");
      rstring.push_str("s love eating mice");
      assert_eq!("cats love eating mice", *rstring);
  }
  ...
  fn test_recycle_arc() {
      ...
      {
        assert_eq!(1, str_pool.size());
        let _rstring = str_pool.new_arc_from("cat");
        assert_eq!(0, str_pool.size());
      }
      assert_eq!(1, str_pool.size());
  }
  ...
  fn test_arc_clone() {
    ...
    {
      assert_eq!(2, str_pool.size());
      let text = str_pool.new_arc_from("cat");
      let text_clone = text.clone();
      assert_eq!(text, text_clone);
      assert_eq!(0, str_pool.size());
    }
  ...
  fn test_detach_arc() {
      ...
      {
        assert_eq!(1, str_pool.size());
        let _string : String = str_pool.new_arc().detach();
        assert_eq!(0, str_pool.size());
      }
      assert_eq!(0, str_pool.size());
  }
  ...
  fn test_attach_arc() {
      ...
      {
        assert_eq!(1, str_pool.size());
        let string: String = str_pool.new_arc().detach();
        assert_eq!(0, str_pool.size());
        let _rstring: ArcRecycled<String> = str_pool.attach_arc(string);
      }
      assert_eq!(1, str_pool.size());
  }
